...
**Append-only:** No UPDATE or DELETE API endpoints will ever touch this collection. Immutability is enforced by the absence of those route handlers.
...
**Append-only:** Activity logs are created through the activity service and there are no UPDATE or DELETE API endpoints for this collection. The frontend also does not provide any way to modify or delete existing activity records.
...
- `GET /api/alerts` — scans all IN_PROGRESS enrollments; the `lastProgressAt` index mitigates this
